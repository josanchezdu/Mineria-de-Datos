...
def render_scraping_tab():
    ...
    Si no hay nuevos, re-consulta los últimos 5 para actualizar citas.<br><br>
     <b>Citas</b>: obtenidas desde la <b>API de Crossref</b> (gratuita, sin bloqueos).
    </div>""", unsafe_allow_html=True)
...
def _run_scraping_nuevos():
    ...
    placeholder.markdown(
        ...
        unsafe_allow_html=True)
    cols_show = ["title", "authors_raw", "publication_date", "topic_label", "citations"]
    df_n = pd.DataFrame(nuevos)[[c for c in cols_show if c in pd.DataFrame(nuevos).columns]].copy()
    df_n.columns = ["Título", "Autores", "Fecha", "Temática", "Citas"][:len(df_n.columns)]
    st.dataframe(df_n, use_container_width=True, hide_index=True)
    st.rerun()

def _verificar_ultimos_5():
    ultimos = query_df(
        "SELECT paper_id, title, doi, url, citations FROM papers ORDER BY publication_date DESC LIMIT 5")
    if ultimos.empty:
    ...
    for i, row in enumerate(ultimos.itertuples(index=False)):
        ...
        try:
            ...
            execute_sql("UPDATE papers SET citations=?, abstract=? WHERE paper_id=?",
                        (detalles["citations"], detalles["abstract"] or "", row.paper_id))
            resultados.append({
                "Título":       row.title[:65] + "…",
            ...
            resultados.append({"Título": row.title[:65] + "…", "Citas antes": row.citations, "Citas nuevas": "—"})
    progress_bar.progress(1.0)
    status.empty()
    st.cache_data.clear()
    st.success(" Citas actualizadas desde Crossref.")
